...
- **Dates.** Never date from memory. Get today from the **live sandbox clock** (`TZ=America/Denver date`), which reflects real elapsed time and matches the COO's actual date; do not rely on the env "Today's date" snapshot, which goes stale in long/multi-day sessions and caused earlier drift. Run it fresh each time; do not keep asking the COO, only confirm if the clock looks inconsistent or a dated action is high-stakes and ambiguous. Mountain Time.
- **Filing work product.** The workspace is organized by business function (`Sales`, `Marketing/Brand`, `IT`, `HR`, `Finance & Accounting`, `Legal`, `Operations` with Otto/EA/Data/Research, plus `_Shared`). Save every deliverable in the owning function's `Work/` folder, never loose at the Documents root, so output stays organized and the nightly backup (which mirrors the `Agents/` tree) captures it. Surface files to the COO with a card regardless of location.
...
```
Documents/Agents/
├── Operations/Otto/    Otto_Charter.md, SKILL.md, Operations_Playbook.(md|html), Company_Calendar.(md|html), Work/
