...
**Type:** Living document · **Version:** 2.4 · **Last updated:** July 16, 2026
...
- Related: Coaching Program KB (`Agents/Operations/Otto/Knowledge/`); agenda archive `Coaches_Meeting_Agendas.docx`; Company Calendar
- Key facts:
...
- File: `Financial_Evaluation_Dashboard_PROTOTYPE.html` (in the `Finance & Accounting/` folder). All figures live in the `REAL INPUTS` block near the top of the script and are keyed by absolute week number. Round every dollar figure to a whole number.
...
- Tools / templates: operator card with the exact prompt and variable map (`Agents/Operations/Otto/Work/Financial_Dashboard_Update_Checklist.md`).
...
### SOP: QuickBooks mechanics (access and export)
- Status: [ACTIVE]
- Owner: COO (COO-only: the QuickBooks login holds export permissions that other staff accounts do not)
- Trigger / frequency: The weekly QuickBooks export (Thursday), and any time QuickBooks access is needed
- Scope: how to reach and log into the QuickBooks machine and run the export. How to actually work in QuickBooks (bookkeeping, entries, reports) is out of scope and will be covered in a separate Finance SOP.
- The QuickBooks computer: it is the server named "Plotter" (it appears as "SA-PC" in Chrome Remote Desktop). Two ways to get in:
  1. In the office: use the machine in the SA main office. The PIN to unlock the computer is saved in the password manager.
  2. Remotely, via Google Chrome Remote Desktop: sign in to the company Google account, go to https://remotedesktop.google.com/access/, select SA-PC, and enter the PIN (also saved in the password manager).
- Log into QuickBooks: once you are on the machine, sign in to QuickBooks with your own unique QuickBooks username and credentials (provided to you). Store them in the password manager. Never record PINs, usernames, or passwords in this document, only where they live.
- Running the export:
  1. Make sure you are logged in to QuickBooks.
  2. Open the "Transaction Pro" icon from the desktop.
  3. In Transaction Pro, click the blue Export button to open a new window.
  4. In the new window, change the end date of the date range to yesterday.
  5. Click "Retrieve Data." This takes a very long time; it is normal for nothing to appear to happen for more than an hour. Do not use QuickBooks or the program while it is working.
  6. Come back later and you should see a series of rows in the grey box. Then click "Export to File."
  7. Save it directly to the QB Extracts folder on SharePoint (InternalSystems, SemArts New Systems, QB Extracts): https://datacentric.sharepoint.com/:f:/s/staff/IgAJrldSuioERZGSXSJP92tQAcK3aJ_eZps_bmWRBeW3ZS4?e=YCmXiz . Name the file "QB Export M.D.YY".
  8. Notify Eddie and Colton via Microsoft Teams that a new export is ready.
- Related: IT & Access Management ("Physical IT resources"); the forthcoming Finance SOP for working in QuickBooks; Company Calendar (weekly Thursday QuickBooks export).
...
- Related: Company Calendar (weekly Monday cash & staff plan); AR schedule (to be documented in a future Finance SOP); Financial Evaluation Dashboard (update steps: `Agents/Operations/Otto/Work/Financial_Dashboard_Update_Checklist.md`)
- Cash Plan:
...
- **Finance:** Expense reimbursement · Payroll processing (needs review) · Financial Evaluation Dashboard weekly update · Project setup (operational onboarding) · Project closing (financial close) · QuickBooks mechanics (access and export)
