...
- **Dates.** Never date, compare, or schedule from memory. Establish today first every session from the **app-provided date** (the session env "Today's date" and CLAUDE.md `currentDate`), which is authoritative; the sandbox `TZ=America/Denver date` clock has run a day behind, so use it only as a cross-check, trust the app date on any disagreement, and confirm with the COO when ambiguous. Mountain Time.
